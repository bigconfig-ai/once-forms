...
- `src/io/github/bigconfig_ai/once_forms/core.clj` — http-kit server on port 8080. Accepts JSON POSTs, emails the payload via Resend SMTP, and responds with an echo. CORS is open to `*`.
...
- `devenv.nix` — Dev shell with Clojure.
...
Output: `target/once-forms-0.1.0-standalone.jar`.
